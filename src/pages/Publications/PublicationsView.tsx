...
// --- Data Loading ---
const projectFiles = import.meta.glob('../../data/publications/*.json', { eager: true });
const allProjectData = Object.values(projectFiles).map((mod: any) => mod.default);

// Filter: Only Publications (Not Activity)
...
const projectAssets = import.meta.glob(['@/assets/project/*.{png,jpg,jpeg,webp}', '@/assets/Publications/*.{png,jpg,jpeg,webp,pdf}'], { eager: true });

const resolveAssetUrl = (url: string) => {
    if (!url.startsWith('/assets/project/') && !url.startsWith('/assets/Publications/')) return url;
                ...
                <div className="flex-1 flex relative h-[calc(100vh-250px)] min-h-[600px]">
                    ...
                    >
                        <div className="space-y-12 pb-20">
                            <div>
                                <Section title="M.S. (Journal)" items={msResearch} onClick={handleItemClick} selectedId={selectedId} subSection />
                                <Section title="B.S. (Conference)" items={bsResearch} onClick={handleItemClick} selectedId={selectedId} subSection />
                            </div>
                        </div>
                    </motion.div>
                                ...
                                <div ref={DetailContainerRef} className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                                    <div className="max-w-3xl mx-auto">
                                                ...
                                                <div className="ml-auto flex items-center gap-2">
                                                        ...
                                                        if (pdfItem) {
                                                            return (
                                                                <a
                                                                    ...
                                                                    className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow-sm"
                                                                >
                                                    ...
                                                    {(selectedItem.details?.doi || (selectedItem.links && selectedItem.links.length > 0)) && (
                                                        <a
                                                            ...
                                                            className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow-sm"
                                                        >
                                                            Publisher <ExternalLink size={12} className="ml-1" />
                                                        </a>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        {selectedItem.itemData?.notice && (
                                            <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/50 rounded-xl text-yellow-600 dark:text-yellow-400">
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="text-sm font-medium leading-relaxed">
                                                        <RichTextRenderer text={selectedItem.itemData.notice} />
                                                    </div>
                                                </div>
                                            </div>
                                        )}

                                        {/* Media Gallery (Carousel/Stack/Grid for 2) */}
                                        {/* Media Gallery (Carousel/Stack/Grid for 2) */}
                                        {(() => {
                                            // Filter out PDF from gallery display
                                            const displayMedia = selectedItem.media?.filter(m => m.type !== 'pdf') || [];
                                            if (displayMedia.length === 0) return null;

                                            return (
                                                <div className={`mb-8 ${displayMedia.length === 2 ? 'grid grid-cols-1 md:grid-cols-2 gap-4' : 'space-y-4'}`}>
                                                    {displayMedia.map((media, idx) => (
                                                        <div key={idx} className={`rounded-xl overflow-hidden border border-border shadow-sm bg-white ${displayMedia.length === 2 ? 'h-full flex flex-col' : ''}`}>
                                                            {media.type === 'youtube' && (
                                                            ...
                                                            {media.type === 'image' && (
                                                                <div className={`flex justify-center bg-white p-2 ${displayMedia.length === 2 ? 'flex-1 items-center min-h-[200px]' : ''}`}>
                                                                    <img src={resolveAssetUrl(media.url)} alt={media.caption || "Project Image"} className="max-w-full max-h-full object-contain" />
                                                                </div>
                                                            )}
                                                            {media.caption && (
                                                                <div className="p-3 text-left text-sm text-foreground bg-secondary/50 border-t border-border/10 w-full">
                                                                    {media.caption}
                                                                </div>
                                                            )}
                                                        </div>
                                        ...
                                        <div className="prose prose-neutral dark:prose-invert max-w-none">
                                            ...
                                            {/* Conference Presentation - NEW */}
                                            {selectedItem.itemData?.presentationImage && (
                                                <div className="mt-12 mb-8">
                                                    <h3 className="text-xl font-bold mb-4">Conference Presentation</h3>
                                                    <div className="rounded-xl overflow-hidden border border-border shadow-sm bg-white group">
                                                        <div className="flex justify-center bg-white p-2">
                                                            <img src={resolveAssetUrl(selectedItem.itemData.presentationImage.url)} alt={selectedItem.itemData.presentationImage.caption || "Conference Presentation"} className="max-w-full h-auto object-contain transition-transform duration-500 group-hover:scale-105" />
                                                        </div>
                                                        {selectedItem.itemData.presentationImage.caption && (
                                                            <div className="p-3 text-left text-sm text-foreground bg-secondary/50 border-t border-border/10">
                                                                {selectedItem.itemData.presentationImage.caption}
                                                            </div>
                                                        )}
                            ...
                            <div>
                                ...
                                {item.itemData?.author && (
                                    <div className="mt-3 text-sm text-muted-foreground line-clamp-1">
                                        <span className="font-semibold text-foreground/80 mr-1">Authors:</span>
                                        {item.itemData.author.split(/(\*\*.*?\*\*)/g).map((part: string, i: number) => {
